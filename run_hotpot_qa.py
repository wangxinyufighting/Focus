...
def generate_summac(data_name):
    from data.summac import SummaCBenchmark
    benchmark_val = SummaCBenchmark(benchmark_folder="data/summac_benchmark/", cut="test", data_name=data_name)
                    ...
                    if loss_index not in memo:
                        if span.text not in NER_type and (span.ent_type_ in NER_type or span.pos_ in pos_tag):
                            ...
                            if use_threshold:
                                ...
                                penalty = torch.sum(weight * loss.to('cpu'), dim=0).tolist()
...
def evaluate(loss_data, pooling="mean", top=5, only_noun=True):
    ...
    for loss_thres_idx in range(len(loss_threshold)):
        ...
        # print('source', source)
        # print('target', target)
        print(f"all: auc_pr: {calcu_auc_pr(source, target)}, auc_pr_og:{calcu_auc_pr_og(source, target)}, auroc:{calcu_auc(source, target)}, auroc_og:{calcu_auc_og(source, target)}") 
        for i in source:
...
def calcu_auc_pr_og(source, target):
...
def filter_dataset(dataset):
    if dataset["name"] == 'hotpot_qa':
        return [dataset]

datasets = generate_summac(data_name='test_easy_has_support_llama2-7b-chat-hf_final_label.json')
datasets = filter_dataset(datasets[-1])

def calcu_auc_og(source, target):
    labels = target
    predictions = source
...
if __name__ == "__main__":
    ...
    for model_path in model_paths:
        ...
        for summac_dataset in datasets:
            ...
            label = [d['label'] for d in summac_dataset["dataset"]]
        ...
        for summac_dataset in datasets:
            print(summac_dataset["name"])
            text = [f"{add_type(d['document'])}\nTL;DR\n{add_type(d['claim'])}" for d in summac_dataset["dataset"]]
            label = [d['label'] for d in summac_dataset["dataset"]]
            name = summac_dataset['name']
            summac_dataset = summac_dataset["dataset"]
